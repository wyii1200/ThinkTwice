...
const sparkData = Array.from({ length: 14 }).map((_, i) => ({
  v: 40 + Math.sin(i / 2) * 8 + i * 1.6,
}));
      ...
      <div className="px-5 pb-6">
        <div className="flex items-center justify-between pt-3">
          <div>
            ...
            <div className="text-lg font-bold flex items-center gap-1.5">
              Mochi is watching <Sparkles className="w-4 h-4 text-ai" />
            </div>
          </div>
          ...
          <div className="flex justify-between items-start relative">
            <div>
              ...
              <div className="text-3xl font-black text-white mt-1">
                RM 1,847.<span className="text-xl">22</span>
              </div>
            ...
            <div className="text-right">
              ...
              <Link to="/resilience" className="block">
                ...
                <div className="text-[10px] text-emerald flex items-center gap-0.5 justify-end">
        ...
        <div className="mt-2 flex gap-3 overflow-x-auto scrollbar-hide -mx-5 px-5 pb-1">
          <InsightCard
            ...
            sub="Mostly skipped late-night Grab orders."
          />
          <InsightCard
            tone="ai"
            title="Food spending down 18% this week."
            sub="Keep this rhythm to unlock 'Cafe Hermit' badge."
          />
          <InsightCard
            tone="risk"
            title="Subscription leak detected."
            sub="Spotify Duo · unused for 23 days. RM14.90/mo."
          />
        </div>

        <div className="mt-4 glass-strong rounded-3xl p-4">
          <div className="flex items-center gap-2 text-xs font-semibold text-ai uppercase tracking-wider">
            <Bot className="w-3.5 h-3.5" /> Financial Orchestrator Agent
          </div>
          <div className="mt-2 text-sm font-bold">
            Transactions stream in, AI agents score risk, and the orchestrator selects the next best
            intervention.
          </div>
          <div className="mt-3 grid grid-cols-2 gap-2 text-[11px]">
            <FlowPill label="Risk Agent: High" />
            <FlowPill label="Nudge Agent: Ready" />
            <FlowPill label="Auto-save: Awaiting approval" />
            <FlowPill label="Radar Agent: 3 cheaper alternatives" />
          </div>
        </div>

        <div className="mt-5 grid grid-cols-4 gap-2">
          <Quick to="/nudge" icon={Plus} label="Save Now" tone="emerald" />
        ...
        <div className="mt-5">
          <div className="flex justify-between items-center mb-2">
            ...
            <Link to="/transactions" className="text-xs text-ai">
              See all
            </Link>
          </div>
        ...
        <div className="mt-4 glass rounded-3xl p-4">
          <div className="text-xs text-muted-foreground uppercase tracking-wider">Daily loop</div>
          <div className="mt-2 text-sm font-semibold leading-relaxed">
            Spend → AI analyzes risk → orchestrator selects intervention → user acts → rewards and
            resilience update → tomorrow&apos;s recommendations improve.
          </div>
        </div>
...
function SignalCard({
  ...
  const color = tone === "emerald" ? "text-emerald" : "text-ai";
  return (
    <div className="glass rounded-3xl p-4">
      <Icon className={`w-4 h-4 ${color}`} />
      <div className="text-[10px] text-muted-foreground uppercase tracking-wider mt-2">{label}</div>
...
function InsightCard({
  ...
  tone: "emerald" | "ai" | "risk";
  title: string;
  sub: string;
}) {
  const grad =
    tone === "emerald" ? "bg-grad-emerald" : tone === "ai" ? "bg-grad-ai" : "bg-grad-risk";
  return (
    <div
      className={`shrink-0 w-[260px] rounded-3xl p-4 ${grad} relative overflow-hidden text-white`}
    >
      <div className="absolute -bottom-6 -right-6 w-32 h-20 opacity-30">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={sparkData}>
            <Area dataKey="v" stroke="#fff" fill="#fff" fillOpacity={0.3} strokeWidth={2} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
      <div className="relative">
...
function FlowPill({ label }: { label: string }) {
  return (
    <div className="rounded-2xl bg-white/8 border border-border px-3 py-2 font-medium">{label}</div>
  );
}
...
function Quick({
  ...
  icon: React.ComponentType<{ className?: string }>;
  label: string;
  tone: "emerald" | "ai" | "purple" | "gold";
}) {
  const grad =
    { emerald: "bg-grad-emerald", ai: "bg-grad-ai", purple: "bg-grad-ai", gold: "bg-grad-gold" }[
      tone
    ] ?? "bg-grad-ai";
  return (
...
function Tx({
  ...
  return (
    <div className="px-4 py-3 flex items-center gap-3">
      <div
        className={`w-10 h-10 rounded-xl flex items-center justify-center ${save ? "bg-grad-emerald" : risk ? "bg-grad-risk" : "bg-secondary"}`}
      >
      ...
      <div
        className={`text-sm font-bold ${amt > 0 ? "text-emerald" : risk ? "text-risk" : "text-foreground"}`}
      >
        {amt > 0 ? "+" : ""}RM{Math.abs(amt).toFixed(2)}
      </div>
    </div>
  );
}
