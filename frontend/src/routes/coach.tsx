...
import { Brain, AlertTriangle, MapPin, Moon, Flame, ChevronRight, Sparkles } from "lucide-react";
import { Area, AreaChart, ResponsiveContainer, Tooltip, XAxis, YAxis } from "recharts";

export const Route = createFileRoute("/coach")({ component: Coach });

const burn = Array.from({ length: 7 }).map((_, i) => ({ d: ["M","T","W","T","F","S","S"][i], actual: [22, 31, 18, 45, 52, 38, 28][i], budget: 45 }));
    ...
    <MobileFrame>
      <div className="px-5 pt-3 pb-6">
        ...
        <div className="mt-2 flex items-center gap-2">
          <div className="w-10 h-10 rounded-2xl bg-grad-ai glow-ai flex items-center justify-center"><Brain className="w-5 h-5 text-white" /></div>
          <div>
            <h1 className="text-xl font-black">AI Coach</h1>
            <div className="text-[11px] text-muted-foreground flex items-center gap-1"><Sparkles className="w-3 h-3 text-ai" /> Confidence 94%</div>
          </div>
        </div>
        ...
        <div className="mt-4 rounded-3xl p-5 bg-grad-risk relative overflow-hidden">
          ...
          <div className="flex items-center gap-2 text-white/90 text-xs font-semibold uppercase tracking-wider"><AlertTriangle className="w-3.5 h-3.5" /> Overspend prediction</div>
          <div className="mt-2 text-white text-lg font-bold leading-snug">"At this spending rate, you may exceed your weekly budget in 2 days."</div>
          ...
          <div className="h-36 mt-2">
            <ResponsiveContainer>
              <AreaChart data={burn}>
                ...
                <XAxis dataKey="d" stroke="oklch(0.7 0.02 260)" fontSize={10} axisLine={false} tickLine={false} />
                <YAxis hide domain={[0, 60]} />
                <Tooltip contentStyle={{ background: "oklch(0.21 0.035 260)", border: "1px solid oklch(1 0 0 / 10%)", borderRadius: 12, fontSize: 11 }} />
                <Area dataKey="budget" stroke="oklch(0.74 0.18 155)" strokeDasharray="4 4" fill="transparent" strokeWidth={1.5} />
                <Area dataKey="actual" stroke="oklch(0.68 0.20 285)" fill="url(#g)" strokeWidth={2.5} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="mt-4 glass-strong rounded-3xl p-4">
          ...
          <div className="mt-1 text-sm font-bold">Reduce RM10 spending today to maintain your savings goal.</div>
          <div className="mt-3 flex gap-2">
            <Link to="/nudge" className="flex-1 py-2.5 rounded-2xl bg-grad-emerald text-emerald-foreground font-semibold text-sm text-center">Take action</Link>
        ...
        <div className="mt-4 glass-strong rounded-3xl p-4">
          ...
          <div className="grid grid-cols-7 gap-1">
            {Array.from({ length: 7*6 }).map((_, i) => {
              ...
              return <div key={i} className="aspect-square rounded" style={{ background: `oklch(0.7 0.22 35 / ${intensity * 0.8 + 0.05})` }} />;
            })}
          </div>
...
function Mini({ label, value }: { label: string; value: string }) {
  return <div className="rounded-2xl bg-white/15 backdrop-blur p-2"><div className="text-[10px] text-white/70 uppercase tracking-wider">{label}</div><div className="text-sm font-bold text-white">{value}</div></div>;
}

function Risk({ icon: Icon, label, v, tone }: any) {
